...
# Randomize Ra and Dec, then sample
def patch_sample(a, b, dilute_factor):
    p = np.random.permutation(len(a))
    size = int(len(a)*dilute_factor)
...
RA_ran = randoms['RA'][:20*len(ra_cen_bin)]
DEC_ran = randoms['DEC'][:20*len(ra_cen_bin)]  
...
# Make Patch Centers During First z-Bin Run and Save Them
if i == 0:
    ra_patch, dec_patch = patch_sample(ra_gal_ran_temp, dec_gal_ran_temp, .01)
    patch_cat = treecorr.Catalog(ra=ra_patch, dec=dec_patch, ra_units='degrees', dec_units='degrees', npatch=N_jk)
    patch_cat.write_patch_centers(outfile+'patch_centers.txt')
    centers = patch_cat.patch_centers
    del ra_patch, dec_patch, patch_cat
# Otherwise Load Centers
else:
    x = np.loadtxt(outfile+'patch_centers.txt')[:,1]
    y = np.loadtxt(outfile+'patch_centers.txt')[:,2]
    z = np.loadtxt(outfile+'patch_centers.txt')[:,3]
    centers = np.column_stack([x,y,z])
    del x,y,z

cen_cat = treecorr.Catalog(ra=ra_cen_bin, dec=dec_cen_bin, ra_units='degrees', dec_units='degrees', patch_centers=centers)
ran_cat = treecorr.Catalog(ra=RA_ran, dec=DEC_ran, ra_units='degrees', dec_units='degrees', patch_centers=centers)
gal_cat = treecorr.Catalog(ra=ra_gal_bin, dec=dec_gal_bin, ra_units='degrees', dec_units='degrees', patch_centers=centers)
gal_ran_cat = treecorr.Catalog(ra=ra_gal_ran_temp, dec=dec_gal_ran_temp, ra_units='degrees', dec_units='degrees', patch_centers=centers)
