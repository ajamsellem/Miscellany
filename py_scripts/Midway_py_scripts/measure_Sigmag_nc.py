...
mask = (JK!=jkid)        # Values of True in the mask will go into the fiduciary sample
RA = clusters['RA'][mask]
...
mask_ran = np.full(Z_ran.shape, True)          # Cannot mask on randoms because the randoms are only in 0.2 < z < 0.55
...
#dd.write('./splashback_jk_trial/debug_RM.npz',rr,dr,rd)
