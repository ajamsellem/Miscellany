...
dd = treecorr.NNCorrelation(nbins = nR, min_sep = thmin, max_sep = thmax,
                                        bin_slop = bslop, sep_units = 'arcmin', verbose=0)
...
# CCCCCCCCCCCC
np.savez(outfile, R=R_mid, xi=xi, ave_dens=ave_density, w=rr.weight*RR, nclust=len(ra_cen_bin), ngal=len(ra_gal_bin), RR=rr.weight, DD=dd.weight, DR=dr.weight, RD=rd.weight)
